...
void left_player(mqd_t queue_out, mqd_t queue_in, int max_range) {
    ...
    while (!guessed) {
        while (true) {
            timespec timeout = next_timeout({1, 0});
            ssize_t r = mq_timedreceive(queue_in, (char*)&attempt_buffer, sizeof(int), nullptr, &timeout);

            if (r >= 0) break;
            check_except(r, ETIMEDOUT);
            if (waitpid(0, nullptr, WNOHANG) != 0) exit(0);
        }
...
void start(int rounds, int range_max) {
    ...
    mqd_t queue_write = (pid == 0) ? child : parent;
    mqd_t queue_read  = (pid == 0) ? parent : child;
